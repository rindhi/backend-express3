...
CREATE TABLE `HistoryKerusakan` (
    ...
    `abnormality` VARCHAR(191) NOT NULL,
    `pic_related` VARCHAR(191) NOT NULL,
...
CREATE TABLE `Mesin` (
    ...
    `status_mesin` VARCHAR(191) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Employee` ADD CONSTRAINT `Employee_problemscwsId_fkey` FOREIGN KEY (`problemscwsId`) REFERENCES `ProblemScw`(`id_scw`) ON DELETE SET NULL ON UPDATE CASCADE;
